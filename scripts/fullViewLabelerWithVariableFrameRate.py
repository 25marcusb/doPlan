...
def get_delay_ms():
    # Read live fps value; clamp to avoid division by zero
...
def update_frames():
    ...
    update_progress_ui()
    root.after(get_delay_ms(), update_frames)
...
root.geometry("1100x860")  # slightly taller for fps controls

start_var = tk.IntVar(master=root, value=0)
end_var   = tk.IntVar(master=root, value=total_frames - 1)

# NEW: Live FPS control (default ~33fps if you were using after(30))
fps_var = tk.DoubleVar(master=root, value=33.0)
...
fps_frame.pack(fill="x", pady=(4, 6))

tk.Label(fps_frame, text="Playback FPS").pack(side=tk.LEFT)

fps_value_label = tk.Label(fps_frame, text=f"{fps_var.get():.1f}")
fps_value_label.pack(side=tk.LEFT, padx=(6, 10))

def on_fps_change(value):
    fps_value_label.config(text=f"{float(value):.1f}")
